# MCP Trust Report: github:makeplane/plane-mcp-server
...
**Evidence:** `plane_mcp/tools/work_item_attachments.py:200`
...
**Evidence:** `plane_mcp/tools/work_item_attachments.py:352`
...
Metadata contains zero-width/bidi control characters or long encoded payloads that can hide instructions. (Severity reduced medium→low: this match is in test code — tests/test_oauth_security.py — which does not run as part of the MCP server.)
...
**Evidence:** `plane_mcp/__main__.py:137`
...
**Evidence:** `plane_mcp/aws_secrets.py:24`
...
**Evidence:** `plane_mcp/client.py:43`

```
api_key = <redacted:secret>"PLANE_API_KEY", "")
```
...
**Evidence:** `plane_mcp/server.py:60`

```
client_secret=<redacted:secret>"PLANE_OAUTH_PROVIDER_CLIENT_SECRET", ""),
```
...
**Evidence:** `plane_mcp/storage.py:31`
...
**Evidence:** `plane_mcp/storage.py:85`

```
password = <redacted:secret>"REDIS_PASSWORD")
```
...
**Evidence:** `plane_mcp/storage.py:86`

```
secret_arn = <redacted:secret>"ELASTICACHE_SECRET_ARN")
```
...
**Evidence:** `plane_mcp/storage.py:123`

```
token_key = <redacted:secret>"REDIS_AUTH_TOKEN_KEY", "authToken")
```
...
_Generated by mcp-trust 0.5.3 at 2026-07-08T15:51:46.659Z._
