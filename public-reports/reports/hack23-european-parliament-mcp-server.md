# MCP Trust Report: github:Hack23/European-Parliament-MCP-Server
...
Enables arbitrary shell command execution with the process privileges. (Severity reduced high→medium: this match is in build/dev-tooling code — scripts/test-mcp-server.ts — which does not run as part of the MCP server.)

**Evidence:** `scripts/test-mcp-server.ts:118`
...
**Evidence:** `scripts/test-mcp-server.ts:159`
...
**Evidence:** `scripts/test-mcp-server.ts:236`
...
**Evidence:** `scripts/generate-stats.ts:1428`
...
**Evidence:** `src/clients/ep/baseClient.ts:520`
...
**Evidence:** `src/clients/ep/doceoClient.ts:143`
...
Enables arbitrary shell command execution with the process privileges. (Severity reduced high→low: this match is in test code — src/main.test.ts — which does not run as part of the MCP server.)

**Evidence:** `src/main.test.ts:16`
...
Metadata contains zero-width/bidi control characters or long encoded payloads that can hide instructions. (Severity reduced medium→low: this match is in documentation code — docs/api/assets/hierarchy.js — which does not run as part of the MCP server.)

**Evidence:** `docs/api/assets/hierarchy.js`
...
Metadata contains zero-width/bidi control characters or long encoded payloads that can hide instructions. (Severity reduced medium→low: this match is in documentation code — docs/api/assets/navigation.js — which does not run as part of the MCP server.)

**Evidence:** `docs/api/assets/navigation.js`
...
**Evidence:** `src/utils/auditLogger.ts:138`

```
* const requiredAuthToken = <redacted:secret>'AUDIT_TOKEN'];
```
...
_Generated by mcp-trust 0.5.3 at 2026-07-08T15:52:16.118Z._
