# MCP Trust Report: github:Softeria/ms-365-mcp-server
...
**Evidence:** `bin/modules/generate-mcp-tools.mjs:64`
...
**Evidence:** `bin/modules/generate-mcp-tools.mjs:17`
...
**Evidence:** `test-calendar-fix.js:14`
...
**Evidence:** `src/token-cache-storage.ts:207`
...
**Evidence:** `bin/modules/download-openapi.mjs:38`
...
**Evidence:** `bin/modules/generate-mcp-tools.mjs:59`
...
**Evidence:** `bin/modules/simplified-openapi.mjs:166`
...
**Evidence:** `remove-recursive-refs.js:194`
...
**Evidence:** `src/token-cache-storage.ts:147`
...
**Evidence:** `bin/modules/download-openapi.mjs:31`
...
**Evidence:** `src/lib/graph-resilience.ts:247`
...
Writes or deletes files, potentially outside a scoped workspace. (Severity reduced high→low: this match is in test code — test/logger-permissions.test.ts — which does not run as part of the MCP server.)

**Evidence:** `test/logger-permissions.test.ts:23`
...
Writes or deletes files, potentially outside a scoped workspace. (Severity reduced medium→low: this match is in test code — test/token-cache-storage.test.ts — which does not run as part of the MCP server.)

**Evidence:** `test/token-cache-storage.test.ts:117`
...
Metadata contains zero-width/bidi control characters or long encoded payloads that can hide instructions. (Severity reduced medium→low: this match is in test code — src/__tests__/graph-tools.test.ts — which does not run as part of the MCP server.)

**Evidence:** `src/__tests__/graph-tools.test.ts`

```
/sites/site1/lists/list1/items/item1/driveItem
```
...
Metadata contains zero-width/bidi control characters or long encoded payloads that can hide instructions. (Severity reduced medium→low: this match is in test code — test/path-encoding.test.ts — which does not run as part of the MCP server.)

**Evidence:** `test/path-encoding.test.ts`
...
**Evidence:** `src/auth.ts:635`

```
const oauthTokenFromEnv = <redacted:secret>
```
...
**Evidence:** `src/token-cache-storage.ts:111`
...
**Evidence:** `test/allowed-scopes.test.ts:104`
...
**Evidence:** `test/auth-paths.test.ts:35`
...
Metadata references sending data/credentials to an external destination. (Severity reduced high→low: this match is in test code — test/secrets.test.ts — which does not run as part of the MCP server.)

**Evidence:** `test/secrets.test.ts`
...
_Generated by mcp-trust 0.5.3 at 2026-07-08T15:50:17.915Z._
