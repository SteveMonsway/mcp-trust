# MCP Trust Report: github:kazuph/mcp-taskmanager
...
Writes or deletes files, potentially outside a scoped workspace. (Severity reduced high→low: this match is in test code — tests/bin-entrypoint.test.ts — which does not run as part of the MCP server.)

**Evidence:** `tests/bin-entrypoint.test.ts:75`
...
Writes or deletes files, potentially outside a scoped workspace. (Severity reduced high→low: this match is in test code — tests/taskmanager.test.ts — which does not run as part of the MCP server.)

**Evidence:** `tests/taskmanager.test.ts:29`
...
**Evidence:** `tests/taskmanager.test.ts:33`
...
_Generated by mcp-trust 0.5.3 at 2026-07-08T14:45:46.577Z._
